...
export default socket => {
  const rid = window.location.href.match(/games\/([\s\S]*)$/)[1];

  socket.on("queue_update", ({ usernames }) => {
    $(".trigger").remove();

    if ($(".custom-host-message[style='background-color: teal;']").length === 0) {
      return;
    }

    for (let container of $(".custom-team-container")) {
      if (container.children[0].innerText !== "Spectators") {
        continue;
      }

      for (let i = 1; i < container.children.length; i++) {
        const user = container.children[i];
        ...
        $(element).addClass("inline-color-block-hover trigger")
          .html(trigger)
          .css("display", "inline-block")
          .css("width", "10px")
          .click(() => socket.emit("set_custom_host", rid, usernames.indexOf(username)));
